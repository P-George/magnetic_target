function [longitude, latitude, height, roll, pitch, heading] = get_generate_attitude(data_file,sample_rate)
...
heading = angel_changed(data_file,4,50*sample_rate,heading_angel,sample_rate);
%heading = data(1:end,4);
%figure();
%plot(heading);
%hold on;
pitch = angel_changed(data_file,5,85*sample_rate,pitch_angel,sample_rate);
%pitch = data(1:end,5);
%plot(pitch);
%hold on;
roll = angel_changed(data_file,6,120*sample_rate,roll_angel,sample_rate);
%roll = data(1:end,6);
%plot(roll);
